...
// var allArticlesInCategory = [];

// function setArticles(){
//     try {
//         var filterCategory = window.location.href.split("/")[window.location.href.split("/").indexOf("category")+1].replace(/\..*/gi,"");
//             var tenant = document.querySelector("body#wt-website") ? "wtu" : "wcm"
//             var step1 = function(x){
//                 if(allArticlesInCategory.length == 0){
//                     $.ajax( {
//                         url: '/bin/'+x+'/articles',
//                         success: function ( data ) {
//                             var allArticles = JSON.parse(data);
//                             [].forEach.call(allArticles,function(a,b){
//                                 [].forEach.call(a.categoryList,function(c,d){
//                                     if(c.name == filterCategory){
//                                         allArticlesInCategory.push(a);
//                                     }
//                                 });

//                                 if(b == allArticles.length-1){
//                                     loadMoreArticles();
//                                 }
//                             });
//                         },
//                         error: function ( jpXHR, status, error ) {
//                             console.log( jpXHR, status, error );
//                         },
//                         complete: function () {
...
//     try {
//         var perPg = Number(document.querySelector("[data-itemsperpg]").attributes["data-itemsperpg"].value);
//         var startIndx =  Number(document.querySelector(".filter-articles").children.length);

//         var placeArticle = function(x){
//             var artTypeText = {"read":" ","listen":"","watch":"","":""}
//             if(/\S+/.test(x.readTime) && /\S+/.test(x.articleType) ){
//                 artTypeText["read"] = x.readTime+" Min Read ";
//                 artTypeText["listen"] = x.readTime+" Min Listen ";
//                 artTypeText["watch"] = x.readTime+" Min Watch ";
//             }
//             document.querySelector(".filter-articles").innerHTML +="<li class='_"+x.articleType+"'><span style='background-image:url("+decodeURI(x.images[0])+")'></span><div><div><a title='"+x.title+"' href='"+x.url+"'>"+x.title+"</a><div class='tb _tbHold'><div><div><p>"+ new Date(x.articleDate).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})+"</p></div><div><p>"+artTypeText[x.articleType]+"</p></div></div></div></div></div></li>";
//         }

//         for(var i=startIndx; i < ((startIndx)+perPg) ;i++){
//             try {
//                 if(allArticlesInCategory[i]){
//                     placeArticle(allArticlesInCategory[i]);
//                 }

//                 if(allArticlesInCategory[i+1] == undefined) {
//                     document.querySelector(".filter_pagination").style.display ="none";
//                 }
//             } catch(err){
